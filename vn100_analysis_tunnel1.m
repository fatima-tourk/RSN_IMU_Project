...
corr_data = S*([tunnel1_mag(:,1), tunnel1_mag(:,2)]' - [center_x; center_y]);
...
filtered_mag_yaw = lowpass(corr_mag_yaw,0.001,fs);

figure(2)
hold on
plot(tunnel1_times,tunnel1_mag_yaw,'r-','LineWidth',1)
plot(tunnel1_times,corr_mag_yaw,'y-','LineWidth',1)
plot(tunnel1_times,filtered_mag_yaw, '-g','LineWidth',2)
xlabel('Time Series (s)')
ylabel(['Yaw Angle (' char(176) ')'])
title('Raw and Calibrated Magnetometer Yaw Angle')
legend('Raw Magnetometer Yaw','Calibrated Magnetometer Yaw','Filtered Magnetometer Yaw')
...
heading = deg2rad(filtered_yaw) - deg2rad(160);
